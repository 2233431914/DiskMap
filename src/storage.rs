//! Crash-safe local storage for app preferences.
//!
...
//! against a SIGKILL mid-save. This module provides a thin wrapper that
//! writes our app preferences to a JSON file in the app data directory
//! using a write-to-temp + fsync + rename pattern, so a crash can leave
//! either the old file intact or the new file intact — never an empty
//! or truncated half-written file.
//!
...
/// Filesystem-backed store for `Preferences`. Writes are crash-safe.
#[derive(Debug, Clone)]
...
impl SafeStorage {
    ...
    /// Atomically write `prefs` to disk. The pattern is:
    /// 1. Serialize to JSON.
    ...
    pub fn write(&self, prefs: &Preferences) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(prefs).context("serialize preferences")?;
        {
    ...
    /// Read preferences from disk with crash-recovery fallback.
    /// Returns default (empty) preferences if both the main file and
    /// the temp file are missing, empty, or corrupt.
    pub fn read(&self) -> Preferences {
        if let Some(p) = Self::read_from(&self.path) {
            return p;
        }
        if let Some(p) = Self::read_from(&self.tmp_path) {
            return p;
        }
        Preferences::default()
    }

    fn read_from(path: &Path) -> Option<Preferences> {
        let text = fs::read_to_string(path).ok()?;
        if text.trim().is_empty() {
            return None;
        }
        serde_json::from_str::<Preferences>(&text).ok()
    }
}
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        let p = std::env::temp_dir()
            .join(format!("disk-map-storage-test-{pid}-{nanos}-{n}"));
