...
DiskMap 是一款面向 macOS 和 Linux 桌面的原生、本地优先磁盘空间分析工具。它会扫描一个目录树，在内存中聚合文件大小，并用可交互的 squarified treemap 把大文件和大目录直接显示出来。
...
- 安全扫描选项：隐藏文件、symlink 策略、stay-on-filesystem
- 排除规则：`.git`、`node_modules` 和自定义模式
- 基于 notify 的实时文件系统 watch，并对 subtree rescan 做 debounce
- 最近扫描根目录和 pinned roots，并持久化用户可见选项

Headless CLI、Linux desktop metadata 和本地 macOS `.app` 打包路径已经可用。代码库中仍保留只读分析/导出模块和带保护逻辑的 cleanup 模块，但它们不再暴露在主 GUI 中。
...
跨平台 app icon 位于 `assets/icons`。Linux desktop launcher metadata 位于 `packaging/linux`；macOS bundle 脚本默认从同一批 PNG icon 生成 app icon，除非显式提供 `.icns` 文件。
...
使用 `2>/dev/null` 可以隐藏 scanner perf log。CLI 没有 preferences、profiles 或 destructive actions，是只读工具。
...
DiskMap is a native, local-first disk usage analyzer for macOS and Linux. It scans a directory tree, aggregates file sizes in memory, and renders an interactive squarified treemap so large files and folders become visible immediately.
...
The headless CLI, Linux desktop metadata, and local macOS `.app` packaging path are available. The codebase still contains read-only analysis/export modules and guarded cleanup logic, but those are no longer exposed in the main GUI.
...
Cross-platform app icons live in `assets/icons`. Linux desktop launcher metadata lives in `packaging/linux`; the macOS bundle script derives its app icon from the same PNG assets unless an explicit `.icns` file is provided.
...
Use `2>/dev/null` to silence the scanner's perf log. The CLI has no preferences, profiles, or destructive actions; it is read-only.
